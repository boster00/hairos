// ARCHIVED: Original path was libs/monkey/tools/metering.js
...
export async function meterGrant(supabase, { userId, action, creditAmount, idempotencyKey, meta, target }) {
  ...
  if (error) {
    console.error('[credit_ledger] meterGrant: meter_grant RPC error', { message: error.message, code: error.code });
    throw error;
  }
  const row = Array.isArray(data) ? data[0] : data;
  if (!row) {
    console.error('[credit_ledger] meterGrant: RPC returned no row', { data });
